...
[
  if test "x[$]$1SYSROOT" != "x"; then
    ...
    $1CPPFLAGS="[$]$1CPPFLAGS [$]$1EXTRA_CFLAGS [$]$1SYSROOT_CFLAGS"
...
[
  ...
  if test "x$OPENJDK_TARGET_OS" = xlinux; then
    ...
    STRIPFLAGS="-x"
  elif test "x$TOOLCHAIN_NAME" = xxcode; then
    STRIPFLAGS="-S"
  elif test "x$OPENJDK_TARGET_OS" = xaix; then
    STRIPFLAGS="-X32_64"
  fi
...
[
  ...
  BUILD_SET_EXECUTABLE_ORIGIN="$SET_EXECUTABLE_ORIGIN"
  BUILD_SET_SHARED_LIBRARY_ORIGIN="$SET_SHARED_LIBRARY_ORIGIN"
  if test "x$OPENJDK_TARGET_OS" = xandroid; then
    SET_EXECUTABLE_ORIGIN=
    SET_SHARED_LIBRARY_ORIGIN=
  fi
  ...
  AC_SUBST(BUILD_SET_EXECUTABLE_ORIGIN)
  AC_SUBST(BUILD_SET_SHARED_LIBRARY_ORIGIN)
...
[
  ...
  if test "x$OPENJDK_$1_OS" = xlinux; then
  ...
  elif test "x$OPENJDK_$1_OS" = xandroid; then
    $2JVM_CFLAGS="[$]$2JVM_CFLAGS -DLINUX"
    ...
    $2COMMON_CCXXFLAGS_JDK="[$]$2COMMON_CCXXFLAGS_JDK -DMACOSX -D_ALLBSD_SOURCE -D_DARWIN_UNLIMITED_SELECT"
  ...
  elif test "x$OPENJDK_$1_OS" = xios; then
    $2COMMON_CCXXFLAGS_JDK="[$]$2COMMON_CCXXFLAGS_JDK -DMACOSX -D_ALLBSD_SOURCE -D_DARWIN_UNLIMITED_SELECT"
    $2JVM_CFLAGS="[$]$2JVM_CFLAGS -D_ALLBSD_SOURCE"
    $2JVM_CFLAGS="[$]$2JVM_CFLAGS -D_DARWIN_C_SOURCE -D_XOPEN_SOURCE"
    $2JVM_CFLAGS="[$]$2JVM_CFLAGS -fno-rtti -fno-exceptions -fvisibility=hidden \
        -mno-omit-leaf-frame-pointer -mstack-alignment=16 -pipe -fno-strict-aliasing \
        -fno-omit-frame-pointer"
    $2JVM_ASFLAGS="[$]$2JVM_ASFLAGS $EXTRA_CFLAGS"
  ...
  elif test "x$OPENJDK_$1_OS" = xmacosx || test "x$OPENJDK_TARGET_OS" = "xios"; then
  ...
  if test "x$OPENJDK_$1_OS" = xmacosx; then
    ...
    $2JVM_ASFLAGS="[$]$2JVM_ASFLAGS -mmacosx-version-min=\$(MACOSX_VERSION_MIN) "
  fi

  # Setup some hard coded includes
  if test "x$OPENJDK_$1_OS" = xandroid ; then
    $2COMMON_CCXXFLAGS_JDK="[$]$2COMMON_CCXXFLAGS_JDK \
        -I${JDK_TOPDIR}/src/java.base/linux/native/include -DLINUX"
  fi
  ...
  if test "x$TOOLCHAIN_TYPE" = xmicrosoft; then
    ...
    if test "x$OPENJDK_$1_OS" = xlinux || test "x$OPENJDK_$1_OS" = xandroid ; then
  ...
  if test "x$TOOLCHAIN_TYPE" = xmicrosoft; then
    ...
    $2LDFLAGS_JDKEXE="[$]$2LDFLAGS_JDKEXE -Wl,--allow-shlib-undefined -pie"
  fi
